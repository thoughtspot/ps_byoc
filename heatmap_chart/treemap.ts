...
function getDataModel(chartModel: ChartModel) {
  ...
  try {
    ...
    const categoryColumnIndex = chartModel.columns.findIndex(
      (col) => col.id === categoryColumn?.id
    );
    const sizeColumnIndex = chartModel.columns.findIndex(
      (col) => col.id === sizeMeasureCol?.id
    );
    const colorColumnIndex = chartModel.columns.findIndex(
      (col) => col.id === colorMeasureCol?.id
    );
            ...
            formatter: function () {
              ...
              if (this.point.index < numberOfLabels) {
                point.options.tooltipData?.forEach((tooltipCol) => {
                  if (
                    dataModel.measureColumns &&
                    dataModel.measureColumns.includes(tooltipCol.columnName)
                  ) {
                    labelHtml += `<span style="display: inline-block; width: 100%; text-align: center;">${
                    ...
                    tuple: [
                      {
                        ...
                        value: categoryValue,
                      }, // Safely accessing category column
                      {
                        columnId: configDimensions?.[1]?.columns?.[0]?.id,
                        value: measureValue,
                      }, // Safely accessing size measure column
                      {
  ...
  try {
    const ctx = getChartContext({
      getDefaultChartConfig: (chartModel: ChartModel): ChartConfig[] => {
        ...
        console.log(
          'getChartContext Attribute columns:',
        ...
        if (attributeColumns.length === 0 || measureColumns.length === 0) {
          throw new Error(
            'Chart configuration error: Missing required attribute or measure columns. Please ensure at least one attribute and one measure are selected.'
          );
        }
